...
public interface VideoRepository extends PagingAndSortingRepository<Video, Integer> {
	...
	@Query("SELECT DISTINCT v FROM Video v JOIN v.lecturer l WHERE CONCAT(v.title, ' ', v.uploader, ' ', v.date, ' ',"
			+ " l.firstName, ' ', l.lastName, ' ', v.date, ' ', v.description, ' ', v.url) Like %?1% AND v.language = ?2")
	...
	@Query("SELECT DISTINCT v FROM Video v JOIN v.lecturer l WHERE CONCAT(v.title, ' ', v.uploader, ' ', v.date, ' ',"
			+ " l.firstName, ' ', l.lastName, ' ', v.date, ' ', v.description, ' ', v.url) Like %?1% AND v.hasSubtitle = ?2")
	...
	@Query("SELECT DISTINCT v FROM Video v JOIN v.lecturer l WHERE CONCAT(v.title, ' ', v.uploader, ' ', v.date, ' ',"
			+ " l.firstName, ' ', l.lastName, ' ', v.date, ' ', v.description, ' ', v.url) Like %?1% AND"
